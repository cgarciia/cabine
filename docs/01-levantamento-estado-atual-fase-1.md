...
**Preenchimento baseado no código do repositório em:** 2026-09-22  
**Branch observada:** `develop`  
**Observação:** respostas marcadas com *não sei* ou deixadas em `___` não foram inventadas. Campos de identificação pessoal ficam para o(a) responsável completar.
...
| Data do preenchimento | 2026-09-22 |
...
| O que está em construção ou quebrado? | Portal `cabine-profissional` + extensões no core (`role`/`crm`/`full_name` em `users`, `GET /people` paginado, `GET /people/{id}`, `GET /people/{id}/visits`, `POST /professionals/register`) — em implementação. Pasta `tests/` no core: não existe. CI (GitHub Actions etc.): não existe no repo. ECG do monitor de pressão: captura existe, mas a leitura ainda tem muito ruído (qualidade insuficiente). |
...
|-------------------|-------------------|------------------------|-------------|
| `users` | email, hashed_password, is_active, role (`operator` \| `professional`), full_name, crm | UUID | Autenticação painel/portal |
...
| `form_submissions` | person_id, module (`health` \| `mental`), status, payload (JSONB), visit_id | UUID | Questionários |
...
| Quais dados pessoais são guardados? | Em `people`: **nome**, **matrícula**, **altura**, **idade**, **data de nascimento**, **sexo**, tipo corporal (`people_type`), peso esperado opcional. **Não** há campos de CPF, e-mail, telefone ou foto na tabela `people`. |
...
| Pergunta | Resposta |
|----------|----------|
| Quantas aplicações existem? São projetos separados ou um só? | **Dois apps front maduros + um em construção:** `cabine-web` (totem + painel no **mesmo** projeto Vite) e `cabine-profissional` (projeto Vite separado). Backend separado. |
...
| A tela guarda algo no navegador? | **Sim:** JWT em `localStorage`; sessão do kiosk (incl. `visitId`) em `sessionStorage`; endereço MAC de dispositivos (oxímetro/pressão) em `localStorage`. Service worker: não observado. |
| O que acontece quando a rede cai no meio de uma avaliação? | não sei (comportamento offline não documentado no código além de erros de API). |
| Como a tela recebe o peso da balança? | WebSocket `/ws/scale` (core faz BLE e envia eventos à tela). |
| Existe impressão? Como? Que impressora? | **Sim** — `window.print()` (diálogo do navegador). Modelo de impressora: **não sei**. |
...
| Como ele é instalado e iniciado? | No código/docs do repo: sobe **manual** (`uvicorn` / `make run` / Makefile). **Não há** no repositório instalador de serviço Windows (NSSM, `.service`, Task Scheduler, etc.). Como está no PC do totem no dia a dia: ___ (preencher: terminal manual, atalho, serviço Windows, etc.). |
| Sistema operacional do totem e versão | Código BLE assume Windows (patches WinRT no core). Versão exata do OS em campo: não sei. |
...
| Problemas conhecidos de hardware/driver | Há documentação em `docs/relatorio-protocolos-balancas.md` e notas no SPEC. Lista operacional atualizada de incidentes: não sei. |
...
| Pergunta | Resposta |
|----------|----------|
| Onde o servidor roda hoje? | Desenvolvimento: máquina local. Produção/piloto (país/host): não sei. |
| Como o deploy é feito? | Repo descreve fluxo local (Docker Postgres + uvicorn + npm). Pipeline CI/CD: **não existe** no repo. |
| Existe Dockerfile/docker-compose? | **docker-compose.yml** no core **só para Postgres**. Dockerfile da API: **não** observado no recorte. |
| Como o totem alcança o servidor? | Em dev: localhost / rede local via IP da API. Produção: não sei. |
| Existe TLS/HTTPS? Como? | Front Vite com certificado **autoassinado** (`@vitejs/plugin-basic-ssl`). TLS da API em produção: não sei. |
| Existe backup do banco? | Volume Docker `postgres_data`. Política de backup/restauração: não sei. |
| Existe monitoramento ou alerta? | **não existe** no código do repo. |
| Variáveis de ambiente e segredos: onde ficam? | `.env` local (template `.env.example`); `.env` não vai para o git. Conteúdo de produção: não sei. |
...
| Existe termo de consentimento? Onde é aceito e guardado? | não sei / **não existe** tabela de consentimento no modelo. |
| Quem tem acesso a dados individuais hoje? | Contas `operator` e `professional` (e a própria pessoa no totem, no escopo da sessão). RH/gestor: não existe. |
| Existe registro de quem consultou o quê (trilha de auditoria)? | **não existe** |
| Prazo de retenção definido? | não sei |
| Existe texto na tela/impresso que sugira diagnóstico, aptidão ou risco? Cole exemplos | Há textos de orientação/triagem no frontend (`advice/`, labels de questionários, findings de BIA). Exemplos literais completos: ver código / prints (anexo). Classificação jurídica: não sei. |
| Quem aprovou os textos e faixas de referência (médico, fonte)? | não sei |
...
| Pergunta | Resposta |
|----------|----------|
| Quais decisões técnicas você tomou que considera importantes e por quê | Montamos inicialmente um SPEC para alinhar escopo e organização. Definimos as bibliotecas de trabalho (ex.: Bleak para BLE no Windows). Desenvolvemos a partir de um modelo HTML disponibilizado, seguindo as perguntas e condições do time de saúde. Arquitetura: API + BLE no `cabine-core`, totem e painel no mesmo SPA (`cabine-web`), visita lógica por `visit_id`, questionários em JSONB; portal profissional como SPA separada no mesmo core. |
...
| O que está planejado para as próximas semanas | Portal profissional (conforme `SPEC-portal-profissional.md`). Outros planos: ___ |
| Dúvidas que você tem sobre o produto ou a arquitetura | ___ |
...
- [ ] Árvore de pastas completa de cada parte (`tree` ou print)
...
- [ ] Textos exibidos ao participante (pastas `advice/`, módulos health/mental — no repo)
