...
type Props = {
    ...
    bpm: number | null;
    active: boolean;
    /** Desenha a onda mesmo sem stream ao vivo (relatório / snapshot). */
    frozen?: boolean;
};
 ...
 */
export function OximeterPulseGraph({ samples, bpm, active, frozen = false }: Props) {
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            const hasLive = frozen
                ? bufferRef.current.length >= 2
                : now - lastSampleAtRef.current < 1500 && bufferRef.current.length >= 2;
